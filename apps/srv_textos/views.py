...
def _resolve_layout_config(
    request_user, card_type, layout_id=None, layout_name=None, layout_override=None
):
...
def _resolve_imagen_path(imagen_url):
    ...
    if "/render/" in imagen_path:
        imagen_path = (
            imagen_url.replace(settings.MEDIA_URL, "")
            .replace("render/", "recortes/")
            .replace("render_", "recorte_")
        )
...
def _fit_text_to_box(
    text, font_path, start_font_size, min_font_size, max_width, ellipsis_enabled=True
):
...
def _measure_habilidad_visual_block(lines, line_height, font_size):
    ...
    for line in lines:
        for token in line:
            ...
            visual_bottom = (
                token_bottom if visual_bottom is None else max(visual_bottom, token_bottom)
            )
...
def _compute_habilidad_visual_content_height(
    habilidad, font_size, max_width, line_spacing, horizontal_padding, card_type=None
):
...
def _compute_layout_metrics(
    ...
    disciplinas=None,
    simbolos=None,
    dynamic_habilidad_from_bottom=False,
    hab_font_size=None,
):
    normalized_card_type = _normalize_card_type(card_type)
    ...
    raw_layout_scope = (
        raw_lay.get("libreria", {}) if normalized_card_type == "libreria" else raw_lay
    )
    ...
    lsi = (
        (layout_scope.get("simbolos") or lay.get("simbolos"))
    ...
    raw_lsi = (
        (raw_layout_scope.get("simbolos") or raw_lay.get("simbolos"))
        if normalized_card_type == "libreria"
        else None
    )
    ...
    habilidad_box = _clamp_box(
        lh.get("box") if has_habilidad_box else None,
        {
    ...
    if (
        is_dynamic_bottom_anchor
        or is_libreria_bottom_anchor_margin
        or is_libreria_legacy_visual_box
    ) and hab_font_size is not None:
    ...
    used_hab_box = {
        ...
        else habilidad_box["y"] + max(0, habilidad_box["height"] - used_hab_box_h),
    ...
    disc_box = _clamp_box(
        ld.get("box") if has_disc_box and isinstance(ld, dict) else None,
        {
    ...
    if normalized_card_type == "libreria" and isinstance(lsi, dict):
        ...
        simbolos_box = _clamp_box(
            lsi.get("box") if has_simbolos_box else None,
            {
        ...
        simbolos_metrics = _compute_symbol_metrics_from_box(
            simbolos_box, icon_count=len(simbolos or [])
        )
    else:
    ...
    cripta_default_box = {
        ...
        "y": max(
            0, used_hab_box["y"] - (pad * 2) - cripta_font_size - int(lcr.get("y_gap", 1) or 1)
        )
    ...
    cripta_box = (
        _clamp_box(lcr.get("box") if has_cripta_box else None, cripta_default_box)
        if isinstance(lcr, dict)
        else None
    )
    ...
    coste_x = (
        int(lco.get("left", card_w - coste_size - int(lco.get("right", 40) or 40)))
        if isinstance(lco, dict)
        else card_w - coste_size - 40
    )
    coste_box = {
        "x": max(0, coste_x),
    ...
    metrics = {
        ...
        "disciplinas": {
            ...
            "source": "box"
            if isinstance(ld, dict) and isinstance(ld.get("box"), dict)
            else "legacy",
        },
        "coste": {
            "box": coste_box,
    ...
    if simbolos_box is not None and simbolos_metrics is not None:
        metrics["simbolos"] = {
            ...
            "anchor_mode": (
                (lsi.get("rules") or {}).get("anchor_mode", "free")
...
def _render_habilidad_text(
    ...
    bg_color=(0, 0, 0),
    box_height=None,
    use_visual_content_height=False,
    card_type=None,
):
    ...
    visual_top_offset, _, visual_content_height = _measure_habilidad_visual_block(
        lines, line_height, font_size
    )
    ...
    overlay_draw.rounded_rectangle(
        [rx, ry, rx + rw, ry + rh], radius=bg_radius, fill=(*bg_color, bg_opacity)
    )
    ...
    if box_height is not None:
        ...
        if use_visual_content_height:
            cur_y = (
                inner_top + max(0, (inner_height - visual_content_height) // 2) - visual_top_offset
            )
        else:
            ...
            if token_types[i] == "symbol":
                ...
                image.alpha_composite(
                    ...
                    (
                        int(cur_x),
                        int(
                            cur_y + max(0, (line_height - winfo["size"]) // 2) - icon_vertical_nudge
                        ),
                    ),
                )
...
def _draw_italic(image, x, y, text, font, color, font_size):
    ...
    tmp = tmp.transform(
        tmp.size, Image.AFFINE, (1, shear, -shear * h / 2, 0, 1, 0), resample=Image.BICUBIC
    )
    image.alpha_composite(tmp, (x, y))


def _render_habilidad_text_libreria(
    image,
    text,
    x,
    y,
        ...
        for tok in line:
            if tok["type"] == "text":
                ...
                image.alpha_composite(
                    ...
                    (
                        int(cx),
                        int(cy + max(0, (line_height - tok["size"]) // 2) - icon_vertical_nudge),
                    ),
                )
                cx += tok["size"] + tok["gap"]
        cy += line_height
...
def _render_carta(
    ...
    simbolos_box_bottom = simbolos_y + int(
        simbolos_box.get("height", simbolos_size * max(1, len(simbolos or [])))
    )
    ...
    hab_box_h = (
        int(hab_box.get("height"))
        if hab_box
        else (int(card_h * lh["box_bottom_ratio"]) - hab_y if "box_bottom_ratio" in lh else None)
    )
    ...
    if nombre:
        ...
        nombre_box = (
            nombre_metrics.get("box")
            or ln.get("box")
            or {
                "x": ln.get("x", 0),
                "y": ln.get("y", 0),
                "width": card_w,
                "height": ln.get("font_size", 40),
            }
        )
        align = nombre_metrics.get("align", "left")
        draw = ImageDraw.Draw(image)
        x = _compute_aligned_x(nombre_box["x"], text_width, align, nombre_box["width"])
        y = int(nombre_box["y"]) + max(
            0,
            (int(nombre_box["height"]) - int(fitted.get("font_size", ln.get("font_size", 40))))
            // 2,
        )
        ...
        if nombre_metrics.get("shadow_enabled", shadow.get("enabled", False)):
            for dx, dy in shadow.get("offsets", []):
                draw.text(
                    (x + dx, y + dy), rendered_name, font=font, fill=shadow.get("color", "black")
                )
    ...
    if card_type == "cripta" and cripta and lcr:
        try:
            ...
            cripta_y = int(cripta_box["y"]) + max(
                0, (int(cripta_box["height"]) - effective_cripta_font_size) // 2
            )
            draw_cripta = ImageDraw.Draw(image)
            draw_cripta.text(
                (cripta_x, cripta_y), str(cripta), font=font_bold, fill=effective_cripta_color
            )
        except Exception as e:
    ...
    if habilidad:
        try:
            _render_habilidad_text(
                ...
                hab_font_size,
                lh["color"],
                bg_opacity=hab_opacity,
                bg_padding=hab_padding,
                vertical_padding=hab_vertical_padding,
                bg_radius=hab_radius,
                line_spacing=hab_line_sp,
                bg_color=tuple(lh["bg_color"]),
                box_height=hab_box_h,
    ...
    if ilustrador and lil:
        try:
            ...
            il_y = int(il_box["y"]) + max(
                0,
                (int(il_box["height"]) - int(il_fit.get("font_size", effective_il_font_size))) // 2,
            )
...
def render_nombre(request):
    ...
    try:
        ...
        dynamic_habilidad_from_bottom = _coerce_bool(
            data.get("dynamic_habilidad_from_bottom", False)
        )
