...
class LayoutUserBootstrapTests(TestCase):
    def test_new_user_gets_default_classic_layouts_for_both_card_types(self):
        ...
        defaults = list(UserLayout.objects.filter(user=user, is_default=True).order_by("card_type"))
...
class LayoutDesktopPackagingTests(SimpleTestCase):
    def test_windows_spec_references_required_seed_resources(self):
        spec = Path(settings.BASE_DIR, "desktop", "windows_launcher.spec").read_text(
            encoding="utf-8"
        )
...
class LayoutApiListCreateTests(TestCase):
    ...
    def test_list_returns_only_current_user_layouts(self):
        ...
        self.assertEqual(
            {layout["id"] for layout in payload["layouts"]},
            {
                own_layout.id,
...
class LayoutConfigBoxSchemaTests(TestCase):
    ...
    def test_normalize_cripta_materializes_disciplina_anchor_box(self):
        ...
        self.assertEqual(
            normalized["disciplinas"]["box"]["width"], normalized["disciplinas"]["size"]
        )
        self.assertEqual(
            normalized["disciplinas"]["box"]["height"], normalized["disciplinas"]["spacing"]
        )
    ...
    def test_normalize_libreria_materializes_box_for_simbolos(self):
        ...
        self.assertEqual(
            normalized["simbolos"]["box"]["height"], normalized["simbolos"]["spacing"] * 3
        )
...
class LayoutPreviewApiTests(TestCase):
    ...
    def test_preview_for_libreria_returns_clean_fixed_source(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json()["imagen_url"], "/media/layout_preview_sources/muestra-de-libreria.png"
        )
        self.assertTrue(
            mock_prepare.call_args.args[0].endswith("static/layouts/images/44. magnum.png")
        )
...
class LayoutEditorStaticAssetTests(SimpleTestCase):
    def test_editor_script_defines_semantic_layer_profiles(self):
        script = Path(settings.BASE_DIR, "static", "layouts", "editor.js").read_text(
            encoding="utf-8"
        )
    ...
    def test_editor_css_defines_visible_labeled_overlays(self):
        stylesheet = Path(settings.BASE_DIR, "static", "layouts", "editor.css").read_text(
            encoding="utf-8"
        )
...
class EndToEndLayoutFlowTests(TestCase):
    def test_user_can_create_edit_set_default_and_render_with_layout(self):
        ...
        with patch(
            "apps.srv_textos.views._render_carta", return_value=("/media/render/e2e.png", None)
        ) as mock_render:
